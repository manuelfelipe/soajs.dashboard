...
module.exports = {
		...
		mongo.findOne(colName, {code: req.soajs.inputmaskData.env}, function (error, envRecord) {
			checkIfError(req, res, {config: config, error: error, code: 733}, function () {
				checkIfError(req, res, {config: config, error: !envRecord, code: 446}, function () {
					...
					else if (driver === 'scheduler - kubernetes') {
						envRecord.deployer.container.scheduler.kubernetes = req.soajs.inputmaskData.scheduler.kubernetes;
					}
	...
	"editDriver": function (config, mongo, req, res) {
		...
		if (driver === 'dockermachine - local') {
			...
			update['$set']['deployer.container.scheduler.kubernetes'] = {};
			update['$set']['deployer.container.scheduler.kubernetes'] = req.soajs.inputmaskData.scheduler.kubernetes;
		}
