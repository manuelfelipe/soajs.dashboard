...
var BL = {
		...
		getEnvInfo(function(){
			getRunningControllers(function(){
				getRunningNginxContainers(function(){
					if(context.nginxes){
						if(deployNewNginx){
							//TODO: if nginxes[0] has certs, cater for the new nginx to be deployed
	...
	"redeployNginx": function (config, soajs, res) {
		...
		function constructDeployerParams(customData, cb) {
			...
			//TODO: add exposed port param if available
			...
			var commands = [];
			dockerParams.variables.forEach(function (oneVar) {
				commands.push('export ' + oneVar + ' && ');
			});
			dockerParams.Cmd[2] = commands.join("") + dockerParams.Cmd[2];
		...
		function reloadContainerAndUpdateDB(cb) {
			...
			deployer.exec(context.deployerConfig, context.containerRecord.cid, BL.model.getDB(), context.dockerParams, function (error, exec) {
				checkIfError(soajs, res, {config: config, error: error, code: 615}, function () {
					exec.start({}, function (error) {
						checkIfError(soajs, res, {config: config, error: error, code: 615}, function () {
