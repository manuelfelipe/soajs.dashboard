...
function deployContainerServiceOrDaemon(config, soajs, res, model, type) {
	...
	function constructDeployerParams(cb) {
		...
		//if dashboard service instance and SOAJS_NO_NGINX is set, pass it to the new dashboard instance
		if (soajs.inputmaskData.name === 'dashboard' && (process.env.SOAJS_NO_NGINX === 'true' || process.env.SOAJS_NO_NGINX === '1')) {
			dockerParams.variables.push("SOAJS_NO_NGINX=" + process.env.SOAJS_NO_NGINX);
		}
...
var BL = {
	...
	"redeployNginx": function (config, soajs, res) {

		var context = {};
		var filePath = __dirname + "/certs/";
		...
		function getContainerRecord(cb) {
			BL.model.getOneContainer(soajs, context.envRecord.code, soajs.inputmaskData.cid, function (error, containerRecord) {
				checkIfError(soajs, res, {config: config, error: error || !containerRecord, code: 600}, function () {
					context.containerRecord = containerRecord;
					return cb();
				});
			});
		}
		...
		function getRunningControllers(cb) {
			BL.model.getHosts(soajs, context.envRecord.code, "controller", function (error, controllers) {
				checkIfError(soajs, res, {config: config, error: error, code: 600}, function () {
		...
		function initializeDeployer(cb) {
			context.deployerConfig = context.containerRecord.deployer;
			checkIfError(soajs, res, {
				config: config,
				error: !context.deployerConfig,
				code: 743
		...
		function constructDeployerParams(customData, cb) {
			var dockerParams = {
				"env": soajs.inputmaskData.envCode.toLowerCase(),
				"name": "nginxapi",
				"image": config.images.nginx,
				"variables": [
					"SOAJS_NX_CONTROLLER_NB=" + context.controllers.length,
					"SOAJS_NX_API_DOMAIN=" + context.envRecord.apiPrefix + "." + context.envRecord.domain, //mydomain.com,
					"SOAJS_NX_SITE_DOMAIN=" + context.envRecord.sitePrefix + "." + context.envRecord.domain
				],
				"Cmd": [
					'bash',
					'-c',
					'cd /opt/soajs/FILES/deployer/; ./soajsDeployer.sh -T nginx -X redeploy -c'
				]
			};

			if (soajs.inputmaskData.ssl) {
				dockerParams.variables.push("SOAJS_NX_API_HTTPS=1");
				dockerParams.variables.push("SOAJS_NX_API_HTTP_REDIRECT=1");
				dockerParams.variables.push("SOAJS_NX_SITE_HTTPS=1");
				dockerParams.variables.push("SOAJS_NX_SITE_HTTP_REDIRECT=1");
			}
		...
		function copyCertificates(cb) {
			...
			function copyCerts(cb) {
				BL.model.getFiles(soajs, {
					"metadata.env": context.envRecord.code.toUpperCase(),
					"metadata.type": "nginx"
				}, function (error, files) {
					checkIfError(soajs, res, {
						config: config,
						error: error || !files || files.length === 0,
						code: 600
					}, function () {
				...
				function step2(files){
					async.each(files, prepareOneFile, function (error) {
						checkIfError(soajs, res, {config: config, error: error, code: 600}, function () {
							return generateArchive(cb);
						});
					});
				}
			}

			function prepareOneFile(oneFile, cb) {
				BL.model.getOneFile(soajs, oneFile._id, function (error, fileData) {
					if (error) {
						return cb(error);
					}

					var fileName = "domainname-chained.";
					switch (oneFile.metadata.label){
						case "certificate":
							fileName += "crt";
							break;
						case "privateKey":
					...
					fs.writeFile(filePath + "/" + fileName, fileData, "utf8", function (error) {
						if (error) {
							return cb(error);
						}
						soajs.log.debug("certificate " + fileName + " prepared.");
						return cb();
					});
				});
			}
		...
		function generateArchive(cb) {
			var certsFolder = filePath.substring(0, filePath.length - 1);
			tar.pack(filePath).pipe(fs.createWriteStream(certsFolder + '.tar'));
			soajs.log.debug("Certificates payload ready to be copied.");
			copyArchive(certsFolder + '.tar', cb);
		}

		function copyArchive(certsTar, cb) {
			soajs.log.debug("Copying certificates payload to container.");
			var src = certsTar;
			var dest = "/opt/soajs/FILES/";
			deployer.copy(context.deployerConfig, context.containerRecord.cid, BL.model.getDB(), src, dest, function (error) {
				checkIfError(soajs, res, {config: config, error: error, code: 616}, function () {
					return cb();
				});
			});
		}

		function reloadContainerAndUpdateDB(cb) {
			soajs.log.debug("redeploying nginx container.");
			soajs.log.debug(context.dockerParams);
			deployer.exec(context.deployerConfig, context.containerRecord.cid, BL.model.getDB(), context.dockerParams, function (error) {
				checkIfError(soajs, res, {config: config, error: error, code: 615}, function () {

					var newParams = context.containerRecord;
					newParams.info.Config.Env = context.dockerParams.variables;
					BL.model.updateContainer(soajs, context.containerRecord.cid, newParams.info, function (error) {
						checkIfError(soajs, res, {config: config, error: error, code: 600}, function () {
							return cb();
						});
					});
		...
		getEnvInfo(function () {
			getContainerRecord(function () {
				getRunningControllers(function () {
					getCustomUIConfigIfApplicable(function (error, customData) {
						checkIfError(soajs, res, {config: config, error: error, code: 615}, function () {
							constructDeployerParams(customData, function () {
								initializeDeployer(function () {
									copyCertificates(function () {
										reloadContainerAndUpdateDB(function () {
											cleanUp(function () {
												return res.json(soajs.buildResponse(null, true));
											});
										});
						...
						getControllerInfo(function () {
							constructDeployerParams(function () {
								deployControllers(0, soajs.inputmaskData.number, context.envRecord, context.dockerParams, function () {
									if (process.env.SOAJS_NO_NGINX === 'true' || process.env.SOAJS_NO_NGINX === '1') {
										return res.json(soajs.buildResponse(null, true));
									}
									else {
	...
	"redeployService": function(config, soajs, res){
		/*
		 - remove the container entry in the hosts collection. {color:red}DO NOT REMOVE THE ENTRY FROM THE DOCKER COLLECTION !{color}
		 - reload registry of all controllers
		 - execute docker.exec and call deployer file to pull updates
		 - execute restart container command = docker.restart
		 -      the restart will not change the container ip
		 - insert a new record in hosts collection for this container
		 - reload registry of all controllers
		 */
			...
			BL.model.getEnvironment(soajs, soajs.inputmaskData.envCode, function (err, envRecord) {
				checkIfError(soajs, res, {config: config, error: err || !envRecord, code: 600}, function () {
					if (envRecord.deployer.type === 'manual') {
						return res.json(soajs.buildResponse({"code": 619, "msg": config.errors[619]}));
					}
		...
		function getServiceHost(env, cb){
			BL.model.getOneHost(soajs, env, soajs.inputmaskData.name, soajs.inputmaskData.ip, soajs.inputmaskData.hostname, function (error, hostRecord) {
				checkIfError(soajs, res, {config: config, error: error, code: 600}, function () {
		...
		function getServiceContainer(env, cb){
			BL.model.getOneContainer(soajs, env, soajs.inputmaskData.hostname, function (error, containerRecord) {
				checkIfError(soajs, res, {config: config, error: error, code: 600}, function () {
					return cb(containerRecord);
				});
			});
		}

		function removeServiceHost(env, cb){
		...
		function reloadControllersRegistries(env, envRecord, cb){
			BL.model.getHosts(soajs, env, "controller", function(error, controllerHosts){
				checkIfError(soajs, res, {config: config, error: error, code: 600}, function () {
					async.each(controllerHosts, reloadControllerRegistry, function(error){
						checkIfError(soajs, res, {config: config, error: error, code: 779}, function () {
							return cb();
						});
					});
				});
			...
			function reloadControllerRegistry(oneController, cb){
				var port = parseInt(envRecord.services.config.ports.controller) + parseInt(envRecord.services.config.ports.maintenanceInc);
				var maintenanceURL = "http://" + oneController.ip + ":" + port + "/reloadRegistry";
		...
		function deployerUpdateServiceCode(envRecord, containerRecord, hostRecord, cb){
			var deployerConfig = containerRecord.deployer;
			var params = {
				"env": envRecord.code,
				"name": hostRecord.name,
				"image": containerRecord.info.Config.Image,
				"variables": containerRecord.info.Config.Env,
				"Cmd": [
					'bash',
					'-c',
					'cd /opt/soajs/FILES/deployer/; ./soajsDeployer.sh -T service -X redeploy -G ' + containerRecord.provider
				]
			};

			params.variables.push("SOAJS_GIT_BRANCH=" + soajs.inputmaskData.branch);
			deployer.exec(deployerConfig, containerRecord.cid, BL.model.getDB(), params, function(error){
				checkIfError(soajs, res, {config: config, error: error, code: 780}, function () {
					return cb();
				});
			});
		}

		function deployerRestartContainer(envRecord, containerRecord, cb){
			var deployerConfig = containerRecord.deployer;
			deployer.restart(deployerConfig, containerRecord.cid, BL.model.getDB(), function(error){
				checkIfError(soajs, res, {config: config, error: error, code: 780}, function () {
					return cb();
				});
			});
		...
		function reInsertServiceHost(hostRecord, cb) {
			BL.model.insertHost(soajs, hostRecord, function (error) {
				checkIfError(soajs, res, {config: config, error: error, code: 600}, function () {
					return cb();
				});
			});
		...
		getEnvInfo(function (envRecord) {
			getServiceHost(envRecord.code, function (hostRecord) {
				getServiceContainer(envRecord.code, function (containerRecord) {
					removeServiceHost(envRecord.code, function () {
						reloadControllersRegistries(envRecord.code, envRecord, function () {
							deployerUpdateServiceCode(envRecord, containerRecord, hostRecord, function () {
								deployerRestartContainer(envRecord, containerRecord, function () {
									reInsertServiceHost(hostRecord, function () {
										reloadControllersRegistries(envRecord.code, envRecord, function () {
											return res.json(soajs.buildResponse(null, true));
										});
									});
					...
					BL.model.getOneContainer(soajs, envRecord.code, soajs.inputmaskData.hostname, function (error, dockerRecord) {
						checkIfError(soajs, res, {
							...
							if (dockerRecord.type === 'controller') {
								rebuildNginx = (process.env.SOAJS_NO_NGINX === 'true' || process.env.SOAJS_NO_NGINX === '1') ? false : true;
							}
