		...
		getRunningControllers(function (controllers) {
			...
			getCustomUIConfigIfApplicable(function (error, customData) {
				checkIfError(req, res, {config: config, error: error, code: 600}, function () {
					...
					checkIfError(req, res, {config: config, error: !envRecord.deployer.type || !envRecord.deployer.selected, code: 743}, function () {
						var deployerConfig = envRecord.deployer;
						var driver = deployerConfig.selected.split(".");
						checkIfError(req, res, {
							config: config,
							error: !deployerConfig[driver[0]] || !deployerConfig[driver[0]][driver[1]] || !deployerConfig[driver[0]][driver[1]][driver[2]],
							code: 743
						}, function () {
...
module.exports = {
	...
	"stop": function (config, mongo, req, res) {
		var dockerColCriteria = {
			'env': req.soajs.inputmaskData.env.toLowerCase(),
			'hostname': req.soajs.inputmaskData.hostname
		};

		mongo.findOne("environment", {code: req.soajs.inputmaskData.env.toUpperCase()}, function (error, envRecord) {
			checkIfError(req, res, {config: config, error: error, code: 600}, function () {
				checkIfError(req, res, {config: config, error: envRecord.deployer.type === "manual", code: 621}, function () {
					mongo.findOne("docker", dockerColCriteria, function (error, dockerRecord) { //getting deployer info
						mongo.update("docker", dockerColCriteria, {'$set': {running: false}}, function (error) { //setting running state to false
							checkIfError(req, res, {config: config, error: error, code: 600}, function () {
								deployer.stop(dockerRecord.deployer, dockerRecord.cid, mongo, function (error) {
									checkIfError(req, res, {config: config, error: error, code: 600}, function () {
										req.soajs.log.debug(req.soajs.inputmaskData.hostname + " has been stopped successfully");
	...
	"start": function (config, mongo, req, res) {
		var dockerColCriteria = {
			'env': req.soajs.inputmaskData.env.toLowerCase(),
			'hostname': req.soajs.inputmaskData.hostname,
			'running': true
		...
		mongo.findOne("environment", {code: req.soajs.inputmaskData.toUpperCase()}, function (error, envRecord) {
			checkIfError(req, res, {config: config, error: error, code: 600}, function () {
				checkIfError(req, res, {config: config, error: envRecord.deployer.type === "manual", code: 621}, function () {
					mongo.findOne("docker", dockerColCriteria, function (error, dockerRecord) {
						checkIfError(req, res, {config: config, error: error, code: 600}, function () {
							checkIfError(req, res, {config: config, error: !dockerRecord, code: 600}, function () {
								deployer.start(dockerRecord.deployer, dockerRecord.cid, mongo, function (error) {
									checkIfError(req, res, {config: config, error: error, code: 600}, function () {
										req.soajs.log.debug(req.soajs.inputmaskData.hostname + " has been started successfully");
										return res.jsonp(req.soajs.buildResponse(null, true));
									});
								});
							});
	...
	"restart": function (config, mongo, req, res) {
		var dockerColCriteria = {
			'env': req.soajs.inputmaskData.env.toLowerCase(),
			'hostname': req.soajs.inputmaskData.hostname
		};
		...
		mongo.findOne("environment", {code: req.soajs.inputmaskData.env.toUpperCase()}, function (error, envRecord) {
			checkIfError(req, res, {config: config, error: error, code: 600}, function () {
				checkIfError(req, res, {config: config, error: envRecord.deployer.type === "manual", code: 621}, function () {
					mongo.findOne("docker", dockerColCriteria, function (error, dockerRecord) {
						checkIfError(req, res, {config: config, error: error, code: 600}, function () {
							deployer.restart(dockerRecord.deployer, dockerRecord.cid, mongo, function (error) {
								checkIfError(req, res, {config: config, error: error, code: 600}, function () {
									req.soajs.log.debug(req.soajs.inputmaskData.hostname + " has been stopped restarted");
									return res.jsonp(req.soajs.buildResponse(null, true));
								});
							});
						});
								...
								mongo.findOne("docker", criteria, function (error, response) {
									checkIfError(req, res, {config: config, error: error, code: 603}, function () {
										...
										if (response) {
											...
											checkIfError(req, res, {config: config, error: true, code: 603}, function () {});
										}
									});
								});
								break;
	...
	"getZombieContainerLogs": function (config, mongo, req, res) {
		var criteria = {
			env: req.soajs.inputmaskData.envCode.toLowerCase(),
			cid: req.soajs.inputmaskData.cid
		};
		...
		mongo.findOne("docker", criteria, function (error, response) {
			checkIfError(req, res, {config: config, error: error, code: 774}, function () {
