...
var BL = {
	...
	"redeployNginx": function (config, soajs, res) {
			...
			function prepareOneFile(oneFile, cb) {
				BL.model.getOneFile(soajs, oneFile._id, function (error, fileData) {
					...
					fs.writeFile(filePath + "/" + fileName, fileData, "utf8", function (error) {
						...
						generateArchive(cb);
						//end prepare file here
						// soajs.log.debug("copying certificate to container.");
						// var src = filePath + fileName;
						// var dest = "/opt/soajs/FILES/" + fileName;
						// deployer.copy(context.deployerConfig, context.containerRecord.cid, BL.model.getDB(), src, dest, cb);
					});
				});
			}
		}
		...
		function generateArchive(cb) {
			...
			var file = fs.createReadStream(certsFolder, "utf8");
			var packer = tar.Pack({}).on('error', function (error) {
				return cb(error);
			}).on('end', function () {
				var tarWriteStream = fs.createWriteStream(certsFolder + '.tar');
				pipe = pipe.pipe(tarWriteStream);
				soajs.log.debug("Certificates payload ready to be copied.");
				copyArchive(certsFolder, cb);
			});

			pipe = file.pipe(packer);
		}

		function copyArchive(certsFolder, cb) {
			soajs.log.debug("Copying certificates payload to container.");
			var src = __dirname + certsFolder + '.tar';
			...
			BL.model.getHosts(soajs, env, "controller", function (error, controllerHosts) {
				checkIfError(soajs, res, {config: config, error: error, code: 600}, function () {
					async.each(controllerHosts, reloadControllerRegistry, function (error) {
						checkIfError(soajs, res, {config: config, error: error, code: 600}, function () { //todo: change the error code
							return cb();
						});
					});
				});
